...
export default function Header({
  tool, canvasW, canvasH,
  onCanvasWChange, onCanvasHChange,
  onShuffle, onUndo, onRedo,
  onDownload, onToggleTheme, isDark,
}: Props) {
  return (
    <header style={{
      height: 46, flexShrink: 0, zIndex: 100,
      background: 'var(--s1)', borderBottom: '1px solid var(--b1)',
      display: 'flex', alignItems: 'center', padding: '0 12px', gap: 5,
    }}>
      {/* Logo */}
      <Link href="/" style={{ textDecoration: 'none', marginRight: 6, flexShrink: 0 }}>
      ...
      <div className="hidden sm:flex" style={{ alignItems: 'center', gap: 4, flexShrink: 0 }}>
        ...
        <input
          ...
          style={{ width: 58, background: 'var(--s3)', border: '1px solid var(--b2)', color: 'var(--t1)', padding: '4px 6px', borderRadius: 6, fontSize: 11, fontFamily: 'var(--font-mono)', textAlign: 'right', outline: 'none' }}
        />
      ...
      {/* Theme toggle */}
      <button onClick={onToggleTheme} title={isDark ? 'Switch to light mode' : 'Switch to dark mode'} style={{
      ...
      {/* Download */}
      <button onClick={() => onDownload('png')} style={{
