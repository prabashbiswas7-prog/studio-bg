...
export default function Header({ tool, canvasW, canvasH, onCanvasWChange, onCanvasHChange, onShuffle, onUndo, onRedo, onDownload, onToggleTheme, isDark }: Props) {
  return (
    <header style={{
      ...
      <div className="hidden sm:flex items-center gap-1">
        ...
        <input
          ...
          style={{ width: 58, background: 'var(--s3)', border: '1px solid var(--b2)', color: 'var(--t1)', padding: '4px 6px', borderRadius: 6, fontSize: 11, fontFamily: 'var(--font-mono)', textAlign: 'right' }}
        />
      ...
      <div className="hidden sm:flex items-center gap-1">
        ...
        <Btn onClick={onShuffle} title="Shuffle">
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="16 3 21 3 21 8"/><line x1="4" y1="20" x2="21" y2="3"/><polyline points="21 16 21 21 16 21"/><line x1="4" y1="4" x2="9" y2="9"/></svg>
          Shuffle
        </Btn>
      ...
      {/* Theme toggle */}
      <Btn onClick={onToggleTheme} title="Toggle theme" icon>
        {isDark
...
function Btn({ onClick, title, icon, children }: { onClick: () => void; title?: string; icon?: boolean; children: React.ReactNode }) {
  return (
    <button
      onClick={onClick}
      title={title}
      style={{
        height: 30, padding: icon ? '0' : '0 10px', width: icon ? 30 : 'auto',
      ...
      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = 'var(--s3)'; (e.currentTarget as HTMLElement).style.color = 'var(--t1)' }}
