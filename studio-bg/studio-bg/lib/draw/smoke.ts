...
export function drawSmoke(ctx: CanvasRenderingContext2D, w: number, h: number, p: Params) {
  const colors=p.colors as string[], scale=p.scale as number||2.2
  const octaves=p.octaves as number||6, bright=p.bright as number||1.15
  const img=ctx.createImageData(w,h); const d=img.data; const step=2
  for (let y=0;y<h;y+=step) for (let x=0;x<w;x+=step) {
    let n=fbm(x/w*scale,y/h*scale,octaves); n=(n+1)*.5*bright
