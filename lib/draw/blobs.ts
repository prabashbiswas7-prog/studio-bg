...
export function drawBlobs(ctx: CanvasRenderingContext2D, w: number, h: number, p: Params) {
  ...
  for (let i = 0; i < (p.count as number || 5); i++) {
    ...
    g.addColorStop(0,hexWithAlpha(col,.88)); g.addColorStop((p.softness as number)/100,hexWithAlpha(col,.3)); g.addColorStop(1,hexWithAlpha(col,0))
    ctx.filter=`blur(${Math.round(r*.1)}px)`; ctx.fillStyle=g; ctx.fill(); ctx.restore()
  }
}
